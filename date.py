...
def fetch_orders():
    ...
    # Fetch historical orders for the current day
...
def format_order_info(orders, title):
    ...
    if not orders:
        ...
        for order in orders:
            order_time = datetime.fromtimestamp(order.time_setup).strftime('%Y-%m-%d %H:%M:%S')
            message += (
...
async def main():
    ...
    history_orders_message = format_order_info(orders_history, "Today's Order History")
